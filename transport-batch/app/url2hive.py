...
def main(**kwargs):
    ...
    for dfwds in dwList:
        try:
            data=get_data(url=url, headersUserAgent=hua, m=m, rowCode=rowCode,
                     colCode=colCode, dbCode=dbCode, wds=wds, dfwds=dfwds)
    ...
    for x in totalList:
        ...
        ## 写入数据库
        mysql_update(code=code,value=value,wds=wds,year=year,unixTime=unixTime)
